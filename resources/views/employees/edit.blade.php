<x-employees.layout>
    ...
    <div class="my-14">
        <h1 class="text-4xl">Update Employee</h1>
        <div class="w-full">
            <form method="POST" action="{{ route('employees.update', $employee) }}"
                ...
                <div class="mb-6 grid md:grid-cols-2 md:gap-6">
                    <div class="mb-6 md:mb-0">
                        ...
                        <input required name="first_name"
                               ...
                               id="employee-firstName" type="text" placeholder="Write your first name here" value="{{old('first_name', $employee->first_name)}}">
                    ...
                    <div>
                        ...
                        <input required name="last_name"
                               ...
                               id="employee-lastName" type="text" placeholder="Write your last name here" value="{{old('last_name', $employee->last_name)}}">
                ...
                <div class="mb-6">
                    ...
                    <input required name="email"
                           ...
                           id="employee-email" type="email" placeholder="Write your email here" value="{{old('email', $employee->email)}}">
                    ...
                    <div id="dropdownSearch" class="z-10  hidden bg-white rounded-lg shadow w-60 ">
                        <div class="p-3">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="input-group-search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 " placeholder="Search">
                            </div>
                        </div>
                            ...
                            @foreach ($dietaryPreferences as $preference)
                                <li>
                                    <div class="flex items-center p-2 rounded hover-bg-gray-100">
                                        <input name="dietary[]" type="checkbox" value="{{ $preference->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2"
                                            {{ (in_array($preference->id, old('dietary', [])) || $employee->dietaryPreferences->contains('id', $preference->id)) ? 'checked' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <div id="additional-phone-fields">
                        @if (!empty($employee->phone))
                                @if (count(old('phone', [])) > 0)
                                    @foreach(old('phone') as $phoneNumber)
                                        <input type="text" id="phone_{{$loop->index}}" name="phone[]" class="form-control shadow appearance-none border {{$errors->first('phone.'.$loop->index) ? 'border-red-500' : null}} rounded-lg w-full py-2 px-3 text-gray-700 mb-2 leading-tight focus:outline-none focus:shadow-outline" value="{{ $phoneNumber }}" placeholder="Write your phone number">
                                        <p id="errorDelete_{{$loop->index}}" class="text-red-500 text-xs italic mb-4">{{$errors->first('phone.'.$loop->index) ? 'Wrong number' : null}}</p>
                                        <p class="hidden">{{$i++}}</p>
                                    @endforeach
                                @else
                                    @foreach (explode(',', $employee->phone) as $phoneNumber)

                                        <input type="text" id="phone_{{$i}}" name="phone[]" class="form-control shadow appearance-none border {{$errors->first('phone.'.$loop->index) ? 'border-red-500' : null}} rounded-lg w-full py-2 px-3 text-gray-700 mb-2 leading-tight focus:outline-none focus:shadow-outline" value="{{old('phone.'.$i, $phoneNumber)}}" placeholder="Write your phone number">
                                        <p id="errorDelete_{{$i}}" class="text-red-500 text-xs italic mb-4">{{$errors->first('phone.'.$loop->index) ? 'Wrong number' : null}}</p>
                                        <p class="hidden">{{$i++}}</p>
                        ...
                        {{-- Script to remove existing numbers --}}
                    </div>

                    <input type="hidden" id="phone-field-count" name="phone-field-count" value="{{ $i }}">
